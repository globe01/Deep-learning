...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, Fca=False,reduction=16, heads=4, mhsa=False, resolution=None):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, resolution=(32, 32), heads=4):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, heads=heads, mhsa=True)
    ...
    def _make_layer(self, block, planes, num_blocks, stride=1, Fca=False, heads=4, mhsa=False):
        ...
        for idx, stride in enumerate(strides):
            if mhsa == True:
                if idx == 0:
                    ...
                    mhsa = False
            layers.append(block(self.in_planes, planes, stride, Fca, heads=heads, mhsa=mhsa, resolution=self.resolution))
            if stride == 2:
                self.resolution[0] /= 2
                self.resolution[1] /= 2
...
def ResNet50(num_classes=100,resolution=(32, 32), heads=4):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes, resolution=resolution, heads=heads)
...
if __name__ == '__main__':
    # 创建结果保存文件夹
    os.makedirs('new_try_result-Fca23', exist_ok=True)
    ...
    # results_df.to_excel('new_try_result/training_results.xlsx', index=False)
    results_df.to_csv('new_try_result-Fca23/training_results.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig('new_try_result-Fca23/loss_accuracy_plot.png')
    plt.show()
