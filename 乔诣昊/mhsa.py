...
def load_data(args):
    ...
    train_dataset = CIFAR100(root='./data', train=True, transform=train_transform, download=False)
    ...
    test_dataset = CIFAR100(root='./data', train=False, transform=test_transform, download=False)
...
class MHSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("n_batch:",n_batch,"heads:",self.heads,"c//heads:",C//self.heads)
        content_content = torch.matmul(q.permute(0, 1, 3, 2), k)

        content_position = (self.rel_h + self.rel_w).view(1, self.heads, C // self.heads, -1).permute(0, 1, 3, 2)
        # print("content_position1:",content_position.size())
        # print("q:",q.size())
        content_position = torch.matmul(content_position, q)
        # print("content_content2:",content_content.size())
        # print("content_position:",content_position.size())
...
def main():
    ...
    if not args.evaluation:
        criterion = nn.CrossEntropyLoss()
        lr_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=100, T_mult=2, eta_min=0.0001)
...
if __name__ == '__main__':
    ...
    df.to_excel('accuracy_scores.xlsx', index=False)