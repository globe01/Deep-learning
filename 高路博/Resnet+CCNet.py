...
# 确保结果文件夹存在
if not os.path.exists('resnet_ccnet_result'):
    os.makedirs('resnet_ccnet_result')
...
# 3. 定义ResNet-18模型，并添加CrissCrossAttention模块
class ResNet18_CIFAR100_CCA(nn.Module):
    def __init__(self):
        super(ResNet18_CIFAR100_CCA, self).__init__()
        self.model = resnet18(weights=None)  # 不使用预训练权重
        self.model.fc = nn.Linear(self.model.fc.in_features, 100)
        self.cca = CrissCrossAttention(self.model.layer4[-1].conv1.out_channels)  # 添加CrissCrossAttention模块
...
if __name__ == '__main__':
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if results["test_accuracy"][-1] > best_accuracy:
            ...
            torch.save(net.state_dict(), 'resnet_ccnet_result/best_model.pth')
    ...
    plt.savefig('resnet_ccnet_result/loss_accuracy_curves.png')
    plt.show()

    # 9. 保存结果到Excel文件
    df = pd.DataFrame(results)
    df.to_excel('resnet_ccnet_result/training_results.xlsx', index=False)
